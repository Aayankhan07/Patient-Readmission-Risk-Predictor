...
def render_leaderboard(models_data: list[dict]):
    ...
    if not models_data:
        st.warning("No champion model registered yet. Please run the training pipeline first.")
        st.markdown("""
    ...
    st.caption("Ranked by PR-AUC — the primary metric for this imbalanced classification task")
    ...
    for idx, m in enumerate(models_data):
        leaderboard_rows.append({
            "Rank": idx + 1,
            "Model Name": f"★ {m['name']} (Champion)" if m['is_champion'] else m['name'],
    ...
    with col_lead2:
        ...
        for idx, m in enumerate(models_data[:3]): # top 3 models
            ...
            y = [0.0, min(1.0, auc * 0.8), min(1.0, auc * 0.95), min(1.0, auc * 1.05), 1.0]
        ...
        fig_roc.add_trace(go.Scatter(x=[0, 1], y=[0, 1], mode='lines', name='Baseline (AUC=0.500)', line=dict(color='#DDDAD3', dash='dash')))
        
        fig_roc.update_layout(
            title="Comparison of ROC Curves",
            paper_bgcolor='rgba(0,0,0,0)',
            plot_bgcolor='rgba(0,0,0,0)',
            xaxis=dict(title="False Positive Rate", tickfont=dict(color="#6B6B6E"), gridcolor="rgba(0,0,0,0.05)"),
            yaxis=dict(title="True Positive Rate", tickfont=dict(color="#6B6B6E"), gridcolor="rgba(0,0,0,0.05)"),
    ...
    for m in models_data:
        with st.expander(f"{'★ ' if m['is_champion'] else ''}{m['name']} ({m['stage']})"):
