...
class ReadmissionPreprocessor(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X: pd.DataFrame, y=None):
        ...
        for col in self.categorical_cols:
            if col == "age":
                continue
            self.mode_values[col] = (
                X[col].mode().iloc[0] if not X[col].mode().empty else "None"
            )
...
if __name__ == "__main__":
    ...
    processed_dir = os.path.dirname(PREPROCESSOR_PATH).replace(
    ...
    X_val_scaled.assign(readmitted=y_val.values).to_csv(
        os.path.join(processed_dir, "val_processed.csv"), index=False
    )
    X_test_scaled.assign(readmitted=y_test.values).to_csv(
