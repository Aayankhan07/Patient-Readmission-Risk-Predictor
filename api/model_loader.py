...
class ModelState:
    ...
    def store_prediction(self, patient_id: str, preprocessed_df: pd.DataFrame):
        """Caches preprocessed features in Redis or falls back to in-memory dict."""
        if self.use_redis and self.redis_client:
            try:
                df_json = preprocessed_df.to_json(orient="split")
                # Expire after 1 hour (3600 seconds) to avoid Redis memory bloating
                self.redis_client.setex(f"pred:{patient_id}", 3600, df_json)
                return
            except Exception as e:
                print(f"Redis write error: {e}. Falling back to in-memory cache.")
        self.predictions_store[patient_id] = preprocessed_df

    def retrieve_prediction(self, patient_id: str) -> pd.DataFrame:
        """Retrieves preprocessed features from Redis or falls back to in-memory dict."""
        if self.use_redis and self.redis_client:
            try:
                df_json = self.redis_client.get(f"pred:{patient_id}")
                if df_json:
                    return pd.read_json(df_json, orient="split")
            except Exception as e:
                print(f"Redis read error: {e}. Falling back to in-memory cache.")
        return self.predictions_store.get(patient_id)
    ...
    def load(self):
        ...
        try:
            ...
            self.use_redis = True
            print(f"Connected to Redis cache at {redis_host}:6379")
        except Exception as redis_err:
            print(
                f"WARNING: Redis unreachable ({redis_err}). Using in-memory fallback cache."
            )
            self.use_redis = False
        ...
        if os.path.exists(PREPROCESSOR_PATH):
            ...
            print(
                f"Fitted preprocessor not found at {PREPROCESSOR_PATH}. Initializing and fitting dummy preprocessor..."
            )
            # Create dummy data and fit preprocessor
            dummy_raw = generate_synthetic_data(
                "data/raw/diabetic_data.csv", num_samples=100
            )
        ...
        processed_train_path = os.path.join(
            os.path.dirname(PREPROCESSOR_PATH).replace("models", "data/processed"),
            "train_processed.csv",
        )
        if os.path.exists(processed_train_path):
            ...
            print(
                "Processed training split not found. Generating synthetic background dataset..."
            )
        ...
        if tracking_uri.startswith("http"):
            ...
            try:
                ...
                print(
                    f"MLflow tracking server at {tracking_uri} is unreachable. Falling back to local 'mlruns' directory."
                )
        ...
        try:
            # 1. Try loading using modern MLflow 2.x alias 'champion'
            try:
                latest_version = client.get_model_version_by_alias(
                    "readmission_champion", "champion"
                )
                ...
                print(
                    f"Loading champion model from registry alias. Run ID: {run_id}, Type: {model_type}"
                )

                model_uri = "models:/readmission_champion@champion"
                if model_type == "ann":
                ...
                print(
                    f"Could not load via alias: {alias_err}. Trying legacy stages fallback..."
                )

                # 2. Legacy stage fallback
                prod_versions = client.get_latest_versions(
                    "readmission_champion", stages=["Production"]
                )
                if prod_versions:
                    ...
                    print(
                        f"Loading champion model from registry stage. Run ID: {run_id}, Type: {model_type}"
                    )

                    model_uri = "models:/readmission_champion/Production"
                    if model_type == "ann":
                        self.model = mlflow.tensorflow.load_model(model_uri)
                    else:
                        self.model = mlflow.sklearn.load_model(model_uri)
            ...
            try:
                ...
                for exp in experiments:
                    runs = client.search_runs(
                        exp.experiment_id, order_by=["metrics.pr_auc DESC"]
                    )
                    if runs:
                        top_run = runs[0]
                ...
                if best_run:
                    ...
                    print(
                        f"Loading best local run {run_id} (Type: {model_type}, PR-AUC: {best_pr_auc:.4f})"
                    )
        ...
        if self.model is None:
            print(
                "No models found in MLflow. Training and saving a dummy LogisticRegression model..."
            )
