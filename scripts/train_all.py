...
def train_classical_models(
    X_train, y_train, X_val, y_val, X_test, y_test, do_tune, trials
):
    ...
    models_to_train = [
        ...
        "xgboost",
        "stacking_classifier",
    ]  # omitting SVM by default for speed, but can be added
    ...
    for model_name in models_to_train:
        ...
        params = {}
        if do_tune and model_name != "stacking_classifier":
            params = tune_model(
                model_name, X_train.values, y_train.values, n_trials=trials
            )
            print(f"Tuned params for {model_name}: {params}")
        else:
            print("Using default parameters or skipping tuning for ensembling models")
        ...
        if model_name == "logistic_regression":
            ...
            from src.models.classical import get_stacking_classifier

            model = get_stacking_classifier(params)
        ...
        with mlflow.start_run(run_name=model_name):
            ...
            mlflow.set_tag(
                "framework", "scikit-learn" if model_name != "xgboost" else "xgboost"
            )
...
def train_ann_model(X_train, y_train, X_val, y_val, X_test, y_test):
    ...
    callbacks = [
        ...
        tf.keras.callbacks.ModelCheckpoint(
            filepath=best_weights_path, monitor="val_loss", save_best_only=True
        ),
    ]

    # 3. Fit Keras Model
    with mlflow.start_run(run_name="keras_ann"):
...
def main():
    parser = argparse.ArgumentParser(
        description="Master training script for PRRP models."
    )
    parser.add_argument(
        "--model-type",
        type=str,
    ...
    if (
        ...
        X_test_scaled = pd.DataFrame(
            X_test_scaled, columns=preprocessor.feature_columns_
        )

        # Save processed splits locally for DVC or reference
        os.makedirs(processed_dir, exist_ok=True)
        X_train_res.assign(readmitted=y_train_res.values).to_csv(
    ...
    if args.model_type in ["ann", "all"]:
        train_ann_model(
            X_train_res, y_train_res, X_val_scaled, y_val, X_test_scaled, y_test
        )
